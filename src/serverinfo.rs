...
where
    R: Read + Seek,
{
    serverinfo_string(r).map(|str| Settings::from(str.as_str()))
}

fn serverinfo_string<R>(r: &mut R) -> Result<String>
where
    R: Read + Seek,
{
...
mod tests {
    ...
    fn test_serverinfo_string() -> Result<()> {
        assert_eq!(
            serverinfo_string(&mut File::open("tests/files/duel_equ_vs_kaboom[povdmm4]20240422-1038.mvd")?)?,
        ...
        assert_eq!(
            serverinfo_string(&mut File::open("tests/files/duel_holy_vs_dago[bravado]20240426-1659.mvd")?)?,
        ...
        assert_eq!(
            serverinfo_string(&mut File::open("tests/files/4on4_oeks_vs_tsq[dm2]20240426-1716.mvd")?)?,
        ...
        assert_eq!(
            serverinfo_string(&mut File::open("tests/files/ctf_blue_vs_red[ctf5]20240520-1925.mvd")?)?,
